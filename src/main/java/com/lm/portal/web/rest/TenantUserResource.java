...
public class TenantUserResource {
    ...
    public TenantUserResource(TenantUserService tenantUserService, TenantUserQueryService tenantUserQueryService) {
    ...
    public ResponseEntity<List<TenantUserDTO>> getAllTenantUsers(TenantUserCriteria criteria, Pageable pageable) {
        log.debug("REST request to get TenantUsers by criteria: {}", criteria);
        Page<TenantUserDTO> page = tenantUserQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /tenant-users/count} : count all the tenantUsers.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/tenant-users/count")
    public ResponseEntity<Long> countTenantUsers(TenantUserCriteria criteria) {
        log.debug("REST request to count TenantUsers by criteria: {}", criteria);
        return ResponseEntity.ok().body(tenantUserQueryService.countByCriteria(criteria));
    }
